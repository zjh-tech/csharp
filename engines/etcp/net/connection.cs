...
namespace Framework.ETcp
{
    public class Connection : IConnection
    {
        ...
        public void AsyncSend(byte[] datas)
        {
            ...
            {
                Log.ErrorAf("[Net] [Connection] ConnID={0} AsyncSend Not Is Established State", conn_id);
                return;
            }
            ...
            lock (send_queue)
            {
                ...
                if (freeflag)
                {
                    ...
                    {
                        Log.ErrorAf("[Net] [Connection] ConnID={0} BeginSend First Error {1}", conn_id, ex.ToString());
                        close(false);
                    }
                }
            }
        ...
        private void OnSend(IAsyncResult result)
        {
            ...
            {
                Log.ErrorAf("[Net] [Connection] ConnID={0} EndSend Error {1}", conn_id, ex.ToString());
                close(false);
                return;
            }
            ...
            lock (send_queue)
            {
                ...
                if (send_size == cur.Length)
                {
                    ...
                    {
                        Log.ErrorAf("[Net] [Connection] ConnID={0} BeginSend Next Error {1}", conn_id, ex.ToString());
                        close(false);
                        return;
                    }
                }
                ...
                {
                    Log.ErrorAf("[Net] [Connection] ConnID={0} BeginSend Current Package", conn_id);
                    ...
                    {
                        Log.ErrorAf("[Net] [Connection] ConnID={0} BeginSend  Continue Current Package Error {1}", conn_id, ex.ToString());
                        close(false);
                        return;
                    }
                }
                ...
                {
                    Log.ErrorAf("[Net] [Connection] ConnID={0} EndSend Package Error", conn_id);
                }
            }
        }
        #endregion
        ...
        public void DoAsyncReceive()
        {
            if (socket == null)
            {
                Log.ErrorAf("[Net] [Connection] ConnID={0} StartRecvThread Socket Null", conn_id);
                return;
            }
            ...
            {
                Log.ErrorAf("[Net] [Connection] ConnID={0} BeginReceive First  Error {1}", conn_id, ex.ToString());
                close(false);
                return;
            }

            Log.InfoAf("[Net] [Connection] ConnID={0} StartRecvThread Socket Success", conn_id);
        }
        ...
        private void on_begin_receive(IAsyncResult result)
        {
            ...
            {
                Log.ErrorAf("[Net] [Connection] ConnID={0} EndReceive Error {1}", conn_id, ex.ToString());
                close(false);
                return;
            }
            ...
            if (is_conn_state(ConnectionState.ESTABLISHED) == false)
            {
                Log.WarnAf("[Net] [Connection] ConnID={0} EndReceive Not Established State", conn_id);
                return;
            }

            if (bytes == 0)
            {
                Log.InfoAf("[Net] [Connection] ConnID={0} Close Socket", conn_id);
                close(false);
                return;
            }
            else if (bytes > 0)
            {
                ...
                {
                    Log.ErrorAf("[Net] [Connection] ConnID={0} BeginReceive Next  Error {1}", conn_id, ex.ToString());
                    close(false);
                    return;
                }
            }
        ...
        private void close(bool terminate)
        {
            ...
            if (terminate)
            {
                timer_register.AddRepeatTimer((UInt32)TimerID.CHECK_SEND_FINISH_TIMER_ID,(long)TimerDelay.CHECK_SEND_FINISH_TIMER_DELAY, "Connection-CheckSendFishish", check_send_finish, new object[]{terminate});
                timer_register.AddRepeatTimer((UInt32)TimerID.CHECK_SEND_TIMEOUT_TIMER_ID, (long)TimerDelay.CHECK_SEND_TIMEOUT_TIMER_DELAY, "Connection-CheckSendTimeout", check_send_timeout, new object[] { terminate });                
            }
        }

        private void check_send_finish(object[] state)
        {
            bool flag = false;
            lock (send_queue)
            {
        ...
        private void check_send_timeout(object[] state)
        {
            timer_register.KillAllTimer();
            bool terminate = (bool)state[0];
            on_close(terminate);
        ...
        private void on_close(bool terminate)
        {
            if (terminate)
            {
                Log.InfoAf("Net [Connection] ConnID={0} Active Closed", conn_id);
            }
            else
            {
                Log.InfoAf("Net [Connection] ConnID={0} Passive Closed", conn_id);
            }
        ...
        private LoopBuffer loop_buffer = new LoopBuffer(NetDef.MSG_BUFF_SIZE);
        ...
        {
            CHECK_SEND_FINISH_TIMER_ID = 1,
            CHECK_SEND_TIMEOUT_TIMER_ID = 2,
        }

        private enum TimerDelay : long
        {
            CHECK_SEND_FINISH_TIMER_DELAY = 1000 * 2,
            CHECK_SEND_TIMEOUT_TIMER_DELAY = 1000 * 60 * 2,
        }
